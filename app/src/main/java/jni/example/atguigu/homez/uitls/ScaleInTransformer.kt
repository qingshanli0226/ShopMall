...
class ScaleInTransformer : BasePageTransformer {
    ...
    @JvmOverloads
    constructor(minScale: Float, pageTransformer: ViewPager.PageTransformer = NonPageTransformer.INSTANCE) {
        mMinScale = minScale
    ...
    public override fun pageTransform(view: View, position: Float) {
        ...
        if (position < -1) { // [-Infinity,-1)
            ...
            //1-2:1[0,-1] ;2-1:1[-1,0]
            {
                ...
                view.pivotX = pageWidth * (BasePageTransformer.DEFAULT_CENTER + BasePageTransformer.DEFAULT_CENTER * -position)
